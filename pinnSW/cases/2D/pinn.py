...
class PhysicsInformedNN:
    ...
    def __init__(self,
        ...
        self.bal_phys_m = tf.Variable(1.0, name='bal_phys_m')
        self.bal_phys_v = tf.Variable(1.0, name='bal_phys_v')

        # Create save checkpoints / Load if existing previous
        self.ckpt    = tf.train.Checkpoint(step=tf.Variable(0),
                                           model=self.model,
                                           bal_phys=self.bal_phys,
                                           bal_phys_m=self.bal_phys_m,
                                           bal_phys_v=self.bal_phys_v,
    ...
    def train(self,
        ...
        bal_phys_m = tf.constant(self.bal_phys_m.numpy(), dtype='float32')
        bal_phys_v = tf.constant(self.bal_phys_v.numpy(), dtype='float32')
        ...
        for ep in range(ep0, ep0+epochs):
            for ba in range(batches):
                ...
                (loss_data,
                 ...
                 bal_phys,
                 bal_phys_m,
                 bal_phys_v) = self._training_step(x_batch,
                                                 y_batch,
                                                 pde,
                                                 eq_params,
                                                 l_data,
            ...
            self.ckpt.bal_phys_m.assign(bal_phys_m.numpy())
            self.ckpt.bal_phys_v.assign(bal_phys_v.numpy())
            if ep%save_freq==0:
                self.manager.save()

            if ep%ckpt_folder_freq==0:
                self._ckpt_folder(ep)

    @tf.function
    def _training_step(self, x_batch, y_batch,
                      pde, eq_params, lambda_data, lambda_phys,
                      data_mask, bal_phys,bal_phys_m, bal_phys_v, alpha, ba, stds):
        with tf.GradientTape(persistent=True) as tape:
            # Data part
            output = self.model(x_batch, training=True)
            y_pred = output[0]
            aux = [tf.reduce_mean(
                   lambda_data*tf.square( (y_batch[:,ii]-y_pred[:,ii])/stds[ii]) )
                   for ii in range(self.dout)
                   if data_mask[ii]]
            loss_data = tf.add_n(aux)

            # Moment conservation physics part
            equations = pde(self.model, x_batch, eq_params)
            mom_u_loss_eqs  = [tf.reduce_mean(
                         lambda_phys*tf.square(equations[0]))
                         ]
            loss_mom_u_phys = tf.add_n(mom_u_loss_eqs)

            mom_v_loss_eqs  = [tf.reduce_mean(                  
                           lambda_phys*tf.square(equations[1])) 
                           ]
            loss_mom_v_phys = tf.add_n(mom_v_loss_eqs)

            # Mass conservation physics part
            mass_loss_eqs  = [tf.reduce_mean(
                         lambda_phys*tf.square(equations[-1]))
                         ]
            loss_mass_phys = tf.add_n(mass_loss_eqs)
        ...
        # Calculate gradients of moment physics part
        gradients_mom_u_phys = tape.gradient(loss_mom_u_phys,
                    self.model.trainable_variables,
                    unconnected_gradients=tf.UnconnectedGradients.ZERO)
        
        gradients_mom_v_phys = tape.gradient(loss_mom_v_phys,
                self.model.trainable_variables,
                unconnected_gradients=tf.UnconnectedGradients.ZERO)

        # Calculate gradients of physics part
        gradients_mass_phys = tape.gradient(loss_mass_phys,
                    self.model.trainable_variables,
                    unconnected_gradients=tf.UnconnectedGradients.ZERO)
        # Delete tape
        del tape
        ...
        if alpha > 0.0:
            mean_grad_mom_u = get_mean_grad(gradients_mom_u_phys, self.num_trainable_vars)
            mean_grad_mom_v = get_mean_grad(gradients_mom_v_phys, self.num_trainable_vars)
            mean_grad_mass = get_mean_grad(gradients_mass_phys, self.num_trainable_vars)
            
            lhat_mu = mean_grad_mom_u/mean_grad_mass
            lhat_mv = mean_grad_mom_v/mean_grad_mass
            lhat_vu = mean_grad_mom_u/mean_grad_mom_v
            lhat_vm = mean_grad_mass/mean_grad_mom_v

            bal_phys_m = (1.0-alpha)*bal_phys_m + alpha*(lhat_mu + lhat_mv)
            bal_phys_v = (1.0-alpha)*bal_phys_v + alpha*(lhat_vu + lhat_vm)
        
        # Apply gradients to the total physics loss function
        gradients_phys = [g_mom_u + bal_phys_v * g_mom_v + bal_phys_m *g_mass
                     for g_mom_u, g_mom_v, g_mass in zip(gradients_mom_u_phys, gradients_mom_v_phys, gradients_mass_phys)]
    ...
    def _print_status(self, ep, lu, lmomuf, lmomvf, lmassf,
        ...
        if alpha:
            ...
            print(ep, self.bal_phys.numpy(), self.bal_phys_m.numpy(), self.bal_phys_v.numpy(),
                  file=output_file)
            output_file.close()

    def _ckpt_folder(self, ep):
            ckpt_path = f't{str(ep)}'
            if not os.path.exists(ckpt_path):
                os.makedirs(ckpt_path)
            os.popen(f'cp -r ckpt *.dat *.npy {ckpt_path}')
