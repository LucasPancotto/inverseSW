...
class PhysicsInformedNN:
    ...
    def __init__(self,
        ...
        self.bal_mom_phys = tf.Variable(1.0, name='bal_mom_phys')

        # Create save checkpoints / Load if existing previous
        self.ckpt    = tf.train.Checkpoint(step=tf.Variable(0),
                                           model=self.model,
                                           bal_phys=self.bal_phys,
                                           bal_mom_phys=self.bal_mom_phys,
    ...
    def train(self,
        ...
        bal_mom_phys = tf.constant(self.bal_mom_phys.numpy(), dtype='float32')
        ...
        for ep in range(ep0, ep0+epochs):
            for ba in range(batches):
                ...
                (loss_data,
                 ...
                 inv_outputs,
                 bal_phys,
                 bal_mom_phys) = self._training_step(x_batch,
                                                 y_batch,
                                                 pde,
                                                 eq_params,
                                                 l_data,
            ...
            self.ckpt.bal_mom_phys.assign(bal_mom_phys.numpy())
            if ep%save_freq==0:
                self.manager.save()
    ...
    def _training_step(self, x_batch, y_batch,
                      ...
                      data_mask, bal_phys,bal_mom_phys, alpha, ba, stds):
        with tf.GradientTape(persistent=True) as tape:
            ...
            mom_loss_eqs  = [tf.reduce_mean(
                         lambda_phys*tf.square(equations[0]))
                         ]
            loss_mom_phys = tf.add_n(mom_loss_eqs)
        ...
        gradients_mom_phys = tape.gradient(loss_mom_phys,
                    self.model.trainable_variables,
                    unconnected_gradients=tf.UnconnectedGradients.ZERO)
        ...
        if alpha > 0.0:
            mean_grad_mom = get_mean_grad(gradients_mom_phys, self.num_trainable_vars)
            mean_grad_mass = get_mean_grad(gradients_mass_phys, self.num_trainable_vars)
            lhat = mean_grad_mom/mean_grad_mass
            bal_mom_phys = (1.0-alpha)*bal_mom_phys + alpha*lhat
        
        # Apply gradients to the total physics loss function
        gradients_phys = [g_mom + bal_mom_phys*g_mass
                     for g_mom, g_mass in zip(gradients_mom_phys, gradients_mass_phys)]
    ...
    def _print_status(self, ep, lu, lmomf, lmassf,
                     inv_ctes, alpha, verbose=False):
        """ Print status function """

        # Loss functions
        output_file = open(self.dest + 'output.dat', 'a')
        print(ep, f'{lu}', f'{lmomf}', f'{lmassf}',
              file=output_file)
        output_file.close()
        if verbose:
            print(ep, f'{lu}', f'{lmomf}', f'{lmassf}')
        ...
        if alpha:
            ...
            print(ep, self.bal_phys.numpy(), self.bal_mom_phys.numpy(),
                  file=output_file)
            output_file.close()
